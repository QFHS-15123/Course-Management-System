<template>
  ...
  <!--创建card-->
  <el-row :gutter="20" class="el-row" type="flex" v-if="condition==='true'">
    <!--循环：3个card在一行-->
...
<!--        card内容-->
        <div>
          <div class="text item">
            <div class="item_tag" ><span>发布人：</span></div>
            <div class="item_desr"><span>{{news_list["tea_name"][index-1]}}</span></div>
          </div>
          <div class="text item">
...
export default {
  ...
  created() {
    ...
    let pojo = {stu_id : token}
    userApi.getStuNews(pojo).then(res => {
      console.log(res)
      ...
      for (let i=0; i<res.data.length; i++){
        ...
        _this.news_list["tea_name"].push(res.data[i].tea_name)
