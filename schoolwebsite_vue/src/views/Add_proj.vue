  ...
  <div>
    <el-form :model="form" label-width="200px" size="large" class="el-form">
      ...
      <el-form-item label="要求">
        <el-input v-model="form.proj_intro" type="textarea" />
      </el-form-item>
      <!--选择截止时间-->
      <el-form-item label="截止时间">
          <el-date-picker v-model="form.proj_ddl" type="datetime"
                          placeholder="选择时间" value-format="YYYY-MM-DD h:m:s"/>
      </el-form-item>
      <!--按钮：发布-->
      <el-form-item>
...
export default {
  ...
  methods:{
    ...
    onSubmit(){
      ...
      //获取数据（注意使用数据库中的属性名）
      let pojo = {proj_name : _this.form.proj_name, cour_id: _this.form.cour_id,
                  proj_intro: _this.form.proj_intro, proj_uptime : _this.nowDate,
                  proj_ddl : _this.form.proj_ddl}
      //发送大作业数据
      userApi.addProj(pojo).then(res => {
        if (res.data === "ok"){
